...
export function AlertsPanel() {
  ...
  return (
    <div className="flex flex-col gap-6 min-h-0 h-full overflow-y-auto pb-8 pr-2">
      ...
      <section
        className="flex flex-col gap-4 mt-6 animate-fade-in stagger-2"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {Object.entries(ALERT_RULES).map(([key, rules]) => {
            ...
            return (
              <div
                key={key}
                className="glass rounded-xl p-5 flex flex-col gap-4 border-l-4"
              ...
              >
                ...
                <div className="flex flex-col gap-2 mt-2">
                  {rules.map((rule, idx) => (
                    <div
                      key={idx}
                      className="flex items-start justify-between text-sm py-2 border-b border-card last:border-0"
                    >
